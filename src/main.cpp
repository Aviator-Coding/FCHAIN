...
#if defined(NDEBUG)
#error "FChain cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 2 * 60 * 60; // 2hr stake maturity
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // FCHAIN: We use certain sporks during IBD, so check to see if they are
