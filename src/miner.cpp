...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("FChainMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("FChainMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("FChainMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("fchain-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running FChainMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
